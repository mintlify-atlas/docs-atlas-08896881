---
title: "Architecture of the wayland-rs crate stack"
sidebarTitle: "Architecture"
description: "Learn how wayland-rs organizes its 12 crates from raw FFI at the bottom to high-level typed protocol bindings at the top."
---

wayland-rs is not a single crate — it is a carefully layered workspace of 12 crates, each with a precisely scoped responsibility. Understanding how those layers fit together helps you choose the right crates for your project, reason about compile-time features, and perform FFI integration confidently. This page walks through every layer from the raw C bindings at the bottom to the typed, ergonomic protocol objects at the top.
...
These two auxiliary crates wrap specific `libwayland-cursor.so` and `libwayland-egl.so` functionality (via `wayland-sys`) and expose safe Rust APIs:

- **`wayland-cursor`**: load cursor images from XCursor themes, upload pixels into `WlBuffer`s, and retrieve frame timing for animated cursors.
- **`wayland-egl`**: create an `EGLSurface` from a `WlSurface` for OpenGL or Vulkan rendering.
...
| `wayland-cursor` | `wayland-client`, `wayland-sys` |
| `wayland-egl` | `wayland-client`, `wayland-sys` |
