...
<Tabs>
  ...
  <Tab title="dlopen feature">
    Like `system`, but dynamically loads `libwayland-client.so` at *runtime* rather than linking at compile time. The binary can start on systems without Wayland installed and gracefully degrade. Requires `system` to take effect.
    ...
    <Tip>
    `dlopen` depends on `system` to be meaningful. When the library fails to load at runtime, `Connection::connect_to_env()` returns `ConnectError::NoWaylandLib`, which you can use to fall back to a different display system (X11, software renderer, etc.).
    </Tip>
  </Tab>
</Tabs>
