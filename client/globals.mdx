...
### `Dispatch<WlRegistry, GlobalListContents>` requirement

To use `registry_queue_init`, your state must implement `Dispatch<WlRegistry, GlobalListContents>`. This is the implementation that will be called for *dynamic* registry events (globals added or removed after the initial roundtrip):
