...
| `dlopen` | Dynamically loads `libwayland-client.so` at runtime instead of linking at compile time. Your binary starts successfully even when no Wayland library is installed—useful for shipping cross-platform binaries that gracefully fall back when running under X11 or Windows. |
...
For every Wayland object your application processes, you must implement `Dispatch<ObjectType, UserData>` for your state type. The `event()` method is called once per received event:
...
<Tip>
The `Dispatch` trait's first type parameter is the proxy type (`WlRegistry`). The second is the user-data type you attached when the object was created—here `()`. You can have multiple `Dispatch` implementations for the same proxy type as long as the user-data types differ.
</Tip>
