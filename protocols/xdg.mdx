...
The following is the full `simple_window` example from the `wayland-client` crate. It binds `xdg_wm_base` from the registry, creates an `xdg_surface` over a `wl_surface`, promotes it to an `xdg_toplevel`, and handles the mandatory `configure`/`ack_configure` handshake before attaching a buffer.
