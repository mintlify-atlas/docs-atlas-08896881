---
title: "OpenGL and EGL integration with wayland-egl"
...
`wayland-backend` implements [`HasDisplayHandle`][rwh_06::HasDisplayHandle] for `client::Backend` when both the `rwh_06` and `client_system` features are enabled. This lets you pass the display handle to crates such as `wgpu` or `winit` through the standard `raw-window-handle` API.

To obtain a `RawWindowHandle` for the surface side you construct a `WaylandWindowHandle` from the `wl_egl_window` pointer:
