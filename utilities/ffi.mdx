---
title: "FFI and interoperability with C Wayland libraries"
...
| `cursor` | `libwayland-cursor.so` | `wayland_sys::cursor` |
| `egl` | `libwayland-egl.so` | `wayland_sys::egl` |

The `dlopen` feature changes all four from link-time symbols to runtime handles loaded via `dlopen(3)`.
