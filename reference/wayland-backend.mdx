---
title: "wayland-backend low-level API reference"
sidebarTitle: "wayland-backend"
description: "Reference for wayland-backend 0.3: the low-level client and server protocol backends, ObjectData trait, and two backend implementations (rs and sys)."
---
...
| `log` | Route internal diagnostic messages through the `log` crate instead of printing to stderr. |
...
pub trait ObjectData: Any + Send + Sync {
    ...
    // Optional — defaults to "ObjectData { ... }"
    fn debug(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result;

    // Optional — used to back Proxy::data() in wayland-client
