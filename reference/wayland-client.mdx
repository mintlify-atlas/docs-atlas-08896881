---
title: "wayland-client API reference"
sidebarTitle: "wayland-client"
description: "Complete API reference for wayland-client 0.31: Connection, EventQueue, Dispatch, QueueHandle, Proxy, and the globals module."
---
...
The `Dispatch` trait is the central event-handling abstraction. Your state type must implement `Dispatch<I, UserData, State>` for every Wayland interface `I` whose events you want to process.
...
<Note>
  `Dispatch` is implemented on the **user-data type** (second type parameter), not directly on the state. This allows multiple implementations for the same interface with different user-data types, and makes generic delegation straightforward.
</Note>
